...
const LeadTienesEstudios = ({ questions }: any) => {
  const leadQuestions = questions.questions;

  const question = getQuestionLead(
    leadQuestions,
    LEAD_QUESTIONS_GROUP_KEY.TIENES_ESTUDIOS
  )[0];
  ...
  return (
    <>
      {question !== undefined && (
        <DisplayLeadQuestion
          colNames={LEAD_COLUMNS_DISPLAY.TIENES_ESTUDIOS}
          question={question}
          cols={1}
        />
      )}
...
export default LeadTienesEstudios;
