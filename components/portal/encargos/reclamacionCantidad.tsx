...
const EncargoReclamacionCantidad = ({ onSubmit }: any) => {
  const lead = useEstudioStore((state) => state.lead);
  const planId = useEstudioStore((state) => state.planId);
  const [plan, setPlan] = React.useState<any>(null);
  const [loading, setLoading] = React.useState<boolean>(true);
  const answers = useEstudioStore((state) => state.answers);
  const debtValue = useEstudioStore((state) => state.debtValue);
  const [firstDate, setFirstDate] = React.useState<any>(null);
  const [lastDate, setLastDate] = React.useState<any>(null);
  const [courtData, setCourtData] = React.useState<any>(null);
  ...
  const getData = async () => {
    const court: any = answers.filter(
      (f) =>
        f.title?.trim() == "Necesitamos estos datos si lo tienes por la mano"
    )[0].selected;

    setCourtData(JSON.parse(court));

    const dateNofitication: any = answers.filter(
      (f) => f.title?.trim() == "¿Cuándo has recibido la demanda?"
    )[0].selected;

    const daysData = JSON.parse(dateNofitication);
    setFirstDate(new Date(daysData.year, daysData.month, daysData.day));

    const days: any = answers.filter(
      (f) =>
        f.title?.trim() == "¿Qué plazo te otorgan para contestar a la demanda?"
    )[0].selected;

    setLastDate(
      firstDate?.setDate(firstDate.getDate() + Number(getDaysDif(days)))
    );
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="prose max-w-none">
      <h1 className="text-center font-bold text-lg mb-8 text-gray-700">
        HOJA DE ENCARGO PROFESIONAL - PROCEDIMIENTO DE RECLAMACIÓN DE CANTIDAD.
      </h1>
      <div className="mx-auto max-w-4xl text-gray-500 bg-white p-8 rounded-lg border mb-8">
        ...
        <p>
          ...
          Asimismo, también será de necesaria intervención el coste de
          Procurador, que se fija según las siguientes tarifas facilitadas para
          clientes de APTD:
        </p>
        <ol>
          <li>
            Oposición de Procedimiento Monitorio: 30€ + 21% IVA, esto es 36,30
            euros.
          </li>
          <li>
            Contestación de Procedimiento Verbal: 85€ + 21% IVA, esto es 102,85
            euros.
          </li>
          <li>
            Procedimiento Ordinario: 190€ +21% IVA Para aquellos concursos en
            los que el CLIENTE si disponga de bienes, se fijan en 250€ + 21%
            IVA, esto es, 229,90 euros.
          </li>
        </ol>
        <p>
          Estos honorarios podrán ser abonados conjuntamente con los de APTD, si
          el cliente así lo estima conveniente.
        </p>
        <p>
          La intervención de Procurador es obligatoria por Ley, por lo que, si
          el CLIENTE así lo desea, puede designar el Procurador que estime
          conveniente por su cuenta y cargo, sin que APTD sea responsable de su
          abono.
        </p>
        <h3>
      ...
      <div className="mx-auto max-w-4xl text-gray-500 bg-white p-8 rounded-lg border">
        ...
        <h4>DATOS DE LA RECLAMACIÓN DE CANTIDAD RECIBIDA</h4>
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  <tr className="divide-x divide-gray-200">
                    <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">
                      Nº de Autos y Juzgado
                    </td>
                    <td className="whitespace-nowrap p-4 text-sm text-gray-500">
                      {courtData.courtRecords} {courtData.courtNum}
                    </td>
                  </tr>
                  <tr className="divide-x divide-gray-200">
                    <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">
                  ...
                  <tr className="divide-x divide-gray-200">
                    <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">
                      Fecha de notificación del requerimiento que se pretende
                      recurrir
                    </td>
                    <td className="whitespace-nowrap p-4 text-sm text-gray-500">
                      {formatDate(firstDate, "medium", "es")}
                    </td>
                  </tr>
                  <tr className="divide-x divide-gray-200">
                    <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">
                      Fecha FINE para oponerse o realizar alegaciones frente al
                      requerimiento de la demandante:
                    </td>
                    <td className="whitespace-nowrap p-4 text-sm text-gray-500">
                      {formatDate(lastDate, "medium", "es")}
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <h4>HONORARIOS ACORDADOS</h4>
        <p>
          El precio del servicio por los honorarios de APTD se fija en la
          cantidad de <strong>{plan.amount} euros</strong>. De este modo, el
          precio total del servicio queda desglosado como sigue:
        </p>
        <div className="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
...
export default EncargoReclamacionCantidad;
