...
import { addUserAPTDCreditor, getCreditors, getDebTypes } from "apis/creditors";
...
const schema = z.object({
  ...
  creditorPhoneNumber: z.string().optional(),
  typeOfDebt: z.string(),
  contractAmount: z
...
function NewUserCreditorAPTDForm() {
  ...
  const debTypes = useQuery(["debTypes"], getDebTypes, {
    onSuccess(data) {
      setAllDebTypes([
        ...data,
        { name: "Deuda privada", value: "deudaPrivada" },
      ]);
    },
  });

  const watchFields = watch(["judicializedDebt", "isSeized", "typeOfDebt"]);
  ...
  const mutation = useMutation((body: any) => addUserAPTDCreditor(body), {
    onSuccess: () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (data.typeOfDebt == "deudaPrivada") {
      body = {
        clientId: router.query.id,
        creditorName: data.creditorName,
        creditorLocation: data.creditorLocation,
        creditorEmail: data.creditorEmail,
        creditorPhoneNumber: data.creditorPhoneNumber,
        ...
        <div className="bg-primary py-6 px-4 sm:px-6">
          ...
          <div className="mt-1">
            <p className="text-sm text-white/60">
              A continuación rellena la siguiente información.
            </p>
          </div>
        </div>
              ...
              {watchFields[2] != "deudaPrivada" ? (
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="relative mt-1 rounded-md shadow-sm">
                      <input
                        {...register("creditorPhoneNumber")}
                        type="text"
                        id="creditorPhoneNumber"
                        className="block w-full rounded-md border-gray-300 pl-3 pr-12 focus:border-secondary focus:ring-secondary sm:text-sm"
                        aria-describedby="price-currency"
                      />
                    </div>
...
export default NewUserCreditorAPTDForm;
