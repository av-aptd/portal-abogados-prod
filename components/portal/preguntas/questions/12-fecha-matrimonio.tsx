...
const help = [
  `<p>Si no recuerdas la fecha del matrimonio, revisa el certificado matrimonial, documento que te dan al momento de casarte. En dicho documento aparece la fecha del matrimonio.</p>
  <p>Si no encuentras este documento, indica una fecha aproximada.</p>
`,
];

const FechaMatrimonio = () => {
  ...
  const onSubmit = async (data: any) => {
    insertAnswer({
      userId: profile.id,
      groupValue: "fecha del matrimonio",
      groupKey: "fechamatrimonio",
      groupNextKey: "regimeneconomico",
      properties: [
        {
          propertyKey: "marriageDate",
          propertyValue: data.marriageDate,
        },
      ],
    });
    setCurrentQuestion("regimeneconomico");
  };

  return (
    <ComplexSectionBloc>
      <HeaderSection title="Indique la fecha del matrimonio" hasActions>
        <button
