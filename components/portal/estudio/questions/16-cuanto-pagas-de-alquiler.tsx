...
const LeadCuantoPagasAlquiler = ({ questions }: any) => {
  const leadQuestions = questions.questions;

  const question = getQuestionLead(
    leadQuestions,
    LEAD_QUESTIONS_GROUP_KEY.CUANTO_PAGAS_ALQUILER
  )[0];
  ...
  return (
    <>
      {question !== undefined && (
        <DisplayLeadQuestion
          colNames={LEAD_COLUMNS_DISPLAY.CUANTO_PAGAS_ALQUILER}
          question={question}
          cols={1}
          valueType="number"
          suffix="€"
...
export default LeadCuantoPagasAlquiler;
